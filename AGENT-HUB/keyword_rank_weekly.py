...
def gsc_export_age_days(folder: Path) -> int | None:
    match = re.search(r"gsc-performance-(\d{4}-\d{2}-\d{2})$", folder.name)
    if not match:
        return None
    export_date = datetime.strptime(match.group(1), "%Y-%m-%d").date()
    return (datetime.now(UTC).date() - export_date).days
...
def gsc_snapshot(device: str) -> dict[str, str] | None:
    ...
    if not folder:
        return None
    age = gsc_export_age_days(folder)
    if age is None or age > GSC_MAX_AGE_DAYS:
        return None
...
def resolve_rank(device: str) -> dict[str, str]:
    for resolver in (serper_rank, serpapi_rank, ddg_proxy_rank, gsc_snapshot):
...
def rank_delta(current: str, previous: str | None) -> str:
    if not previous:
        return "ilk ölçüm"
    cur = parse_rank(current)
    prev = parse_rank(previous)
    if cur is None or prev is None:
        return f"{previous} → {current}"
    diff = prev - cur
    if diff > 0:
        return f"{previous} → {current} (↑{diff:.2f} sıra iyileşme)"
    if diff < 0:
        return f"{previous} → {current} (↓{abs(diff):.2f} sıra düşüş)"
    return f"{previous} → {current} (değişmedi)"
...
def update_weekly_monitoring(captured_at: str, mobile: dict[str, str], desktop: dict[str, str]) -> Path:
    ...
    lines = [
        ...
        "",
        "### Haftalık delta",
        f"- Mobil: {rank_delta(mobile['rank_position'], prev_mobile.get('rank_position') if prev_mobile else None)}",
        f"- Masaüstü: {rank_delta(desktop['rank_position'], prev_desktop.get('rank_position') if prev_desktop else None)}",
        "",
        "### Notlar",
        ...
        + (f" ({gsc_age} gün önce)" if gsc_age is not None else ""),
