...
HUB_URL = "https://ledajans.com/led-ekran/"
USER_AGENT = (
...
def update_weekly_monitoring(captured_at: str, mobile: dict[str, str], desktop: dict[str, str]) -> Path:
    ...
    ddg_hint = ddg_proxy_rank("mobile")
    ...
    if ddg_hint and mobile["source"] != "ddg_html_proxy":
        lines.append(
            f"- DDG proxy (referans, Google değil): sıra {ddg_hint['rank_position']} — `{ddg_hint['target_url']}`"
        )
    lines.extend(
        [
