...
1) wp-content/mu-plugins/ledajans-perf-patch.php
   -> Repodaki wordpress-mobil-hiz-patch.php TAMAMINI yapistir (mobil CWV).
2) httpdocs/robots.txt
   -> Repodaki robots.txt (crawl-budget kurallari).
...
def deploy_blog_file(
    ...
    path = os.path.join(ROOT, file_rel.replace("/", os.sep))
...
def main() -> int:
    ...
    rc_hub = run_py("deploy-money-pages.py", args.apply)

    print("\n3) RankMath P0 + anasayfa meta...")
    rc_meta = subprocess.run(
        [sys.executable, os.path.join(ROOT, "scripts", "set-rankmath-p0-meta.py")]
    ...
    if args.apply:
        ...
        subprocess.run(
            [sys.executable, os.path.join(ROOT, "scripts", "verify-money-pages-live.py")],
            cwd=ROOT,
        )
