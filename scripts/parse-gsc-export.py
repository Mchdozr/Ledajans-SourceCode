...
OUT_QUERIES = DATA / "gsc-queries-prioritized.csv"
OUT_REPORT = ROOT / "AGENT-HUB" / "REPORTS" / f"{datetime.now(timezone.utc).strftime('%Y-%m-%d')}-gsc-live.md"
...
def main() -> int:
    ...
    if not qfile:
        print(f"GSC sorgu CSV bulunamadı. Şuraya koyun: {DATA}")
