...
| `AGENT-HUB/keyword_rank_weekly.py` | Haftalık `led ekran` SERP ölçümü → `SERP-BASELINE.csv` + `WEEKLY-MONITORING-*.md` | `python3 AGENT-HUB/keyword_rank_weekly.py` |
| `AGENT-HUB/run-keyword-rank-weekly.sh` | SERP ölçüm döngü wrapper (cron) | `bash AGENT-HUB/run-keyword-rank-weekly.sh` |
