...
| `AGENT-HUB/keyword_serp_led_ekran.py` | “led ekran” için ledajans.com sırası (DuckDuckGo HTML proxy; Google CAPTCHA nedeniyle yok) | `python3 AGENT-HUB/keyword_serp_led_ekran.py` |
| `AGENT-HUB/run-keyword-serp-led-ekran.sh` | Üstteki betiğin kabuk sarmalayıcısı (cron’da haftalık çalıştırılabilir) | `bash AGENT-HUB/run-keyword-serp-led-ekran.sh` |
