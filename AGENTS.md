...
| `AGENT-HUB/serp_led_ekran_weekly.py` | `led ekran` için Google (tr) organik sıra anlık görüntüsü — SerpAPI (`SERPAPI_API_KEY`) | `export SERPAPI_API_KEY=... && python3 AGENT-HUB/serp_led_ekran_weekly.py` |
| `AGENT-HUB/run-serp-led-ekran-weekly.sh` | Yukarıdaki betiğin ince sarmalayıcısı | `bash AGENT-HUB/run-serp-led-ekran-weekly.sh` |
