...
- Agents must NOT run git commit, git push, or deploy commands unless explicitly approved by the user.
- Every agent must write output to `AGENT-HUB/REPORTS/<yyyy-mm-dd>-<role>.md`
- If blocked, include `[BLOCKER]` in the report title and summary.
- Never print, commit, or expose credentials from `deploy-to-wordpress.py`.
- `TASKS.md` is the single source of truth for task state (pending/in_progress/done).
