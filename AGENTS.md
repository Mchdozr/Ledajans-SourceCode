...
| `AGENT-HUB/serp_rank_check.py` | Anahtar kelime SERP sırası (led ekran) + haftalık rapor | `bash AGENT-HUB/run-serp-rank.sh` |
| `AGENT-HUB/run-serp-rank.sh` | SERP ölçümü (DDG yedek; `SERPER_API_KEY` ile Google) | `bash AGENT-HUB/run-serp-rank.sh --weekly` |
